...
export default function Board({ tasksByStatus, onDropStatus, onEdit, onToggleDone, onReorder }) {
    ...
    return (e) => {
      ...
      try {
        ...
        if (fromStatus && fromStatus !== newStatus) {
          onDropStatus(Number(id), newStatus).then(updateOrder).catch(updateOrder);
        } else {
          updateOrder();
        }
  ...
  return (
    <div className="kanban">
      <Column title="未完了" onDrop={makeOnDrop("pending")} onDragOver={onDragOver} active={hover.status === "pending"}>
        {sorted.pending.map((t, i) => (
          <Card key={t.id} t={t} status="pending" index={i} />
        ))}
      </Column>
      <Column title="完了" onDrop={makeOnDrop("done")} onDragOver={onDragOver} active={hover.status === "done"}>
        {sorted.done.map((t, i) => (
          <Card key={t.id} t={t} status="done" index={i} />
        ))}
      </Column>
      <Column title="未達" onDrop={makeOnDrop("failed")} onDragOver={onDragOver} active={hover.status === "failed"}>
...
function Column({ title, children, onDrop, onDragOver, active }) {
  return (
    <div className={`kanban-col ${active ? "dragging" : ""}`} onDrop={onDrop} onDragOver={onDragOver}>
