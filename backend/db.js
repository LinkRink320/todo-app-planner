...
// Ensure the directory exists (needed on Railway before volume is mounted)
...
db.serialize(() => {
  ...
  db.all("PRAGMA table_info('tasks')", (err, cols) => {
    ...
    db.serialize(() => {
      ...
      db.run(
        "CREATE INDEX IF NOT EXISTS idx_tasks_user_failed_at ON tasks(line_user_id, status, failed_at)"
      );
      db.run(
