function parse(text) {
  ...
  if (t.startsWith("padd ")) { const name = t.slice(5).trim(); if (!name) return { type: "error", msg: "書式: padd プロジェクト名" }; return { type: "project_add", name }; }
  ...
  if (t.startsWith("lsp ")) { const projectId = Number(t.split(" ")[1]) || 0; if (!projectId) return { type: "error", msg: "書式: lsp {projectId}" }; return { type: "list_project_tasks", projectId }; }
  if (t.startsWith("done ")) { return { type: "done", id: Number(t.split(" ")[1]) || 0 }; }
  if (t.toLowerCase() === "watch here") return { type: "watch_here" };
  if (t.startsWith("prog ")) { const parts = t.split(" "); const id = Number(parts[1]) || 0; const pctStr = (parts[2] || "").replace("%", ""); if (!id || pctStr === "") return { type: "error", msg: "書式: prog {id} {0-100%}" }; const progress = Math.max(0, Math.min(100, Number(pctStr) || 0)); return { type: "progress", id, progress }; }
  return { type: "help" };
}
